...
import { RefreshCw, Loader2, BarChartHorizontal, Trash2, ShieldCheck, ShieldOff, Download, FileText } from 'lucide-react'
...
export default function FacebookPagesList() {
    ...
    const handleBulkAction = async (action: 'enable' | 'disable' | 'delete' | 'applyTemplate') => {
        ...
        const promises = Array.from(selectedIds).map(pageId => {
            if (action === 'delete') {
                return deletePageMutation.mutateAsync({ profileId: Number(profileId), pageId });
            } else {
                const status = action === 'enable' ? 'enabled' : 'disabled';
                return updateSettingsMutation.mutateAsync({ pageId, payload: { status } });
            }
        });
        ...
        <div className="p-6 space-y-6 text-white relative">
            ...
            <div className="flex items-center flex-wrap gap-3">
                ...
                <div className="flex ml-auto gap-3">
                    <button onClick={handleSync} disabled={syncPagesMutation.isPending} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-semibold disabled:bg-gray-500">
                        {syncPagesMutation.isPending ? <Loader2 size={18} className="animate-spin" /> : <RefreshCw size={18} />}
                        Escanear
                    </button>
                    <button onClick={() => router.push(`/projects/${id}/facebook-profiles/${profileId}/pages/import`)} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm font-semibold">
                        <Download size={18} /> Importar Leads
                    </button>
                    <input
                        type="text"
                        placeholder="Pesquisar..."
                        className="bg-gray-800 px-4 py-2 border border-gray-700 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                        value={search}
                        ...
                        <thead className="bg-gray-700 text-sm text-gray-300 uppercase">
                            <tr>
                                <th className="p-4 text-left">
                                    <input ref={selectAllCheckboxRef} type="checkbox" checked={isAllOnPageSelected} onChange={(e) => handleSelectAll(e.target.checked)} className="h-4 w-4 rounded bg-gray-800 border-gray-600 text-blue-500"/>
                                </th>
                                <th className="px-4 py-3 text-left font-semibold">Leitor / Perfil</th>
                                <th className="px-4 py-3 text-center font-semibold">Última Mensagem</th>
                        ...
                        <tbody className="bg-gray-800 divide-y divide-gray-700">
                            {paginated.map((page) => (
                                <tr key={page.id} className={`transition-colors ${selectedIds.has(page.id) ? 'bg-blue-900/20' : 'hover:bg-gray-900/50'}`}>
                                    <td className="p-4">
                                        <input type="checkbox" checked={selectedIds.has(page.id)} onChange={(e) => handleSelectOne(page.id, e.target.checked)} className="h-4 w-4 rounded bg-gray-900 border-gray-600 text-blue-500"/>
                                    </td>
                                    ...
                                    <td className="px-4 py-3 text-center min-w-[200px]">
                                        <div className="inline-block max-w-[250px] bg-gray-700/50 rounded-lg px-3 py-1.5 text-sm text-gray-200 italic border border-gray-600 truncate">
                                            {page.last_conversation_message ? `"${page.last_conversation_message}"` : <span className="text-gray-500">Sem mensagens</span>}
                                        </div>
                                    </td>
                                    <td className="px-4 py-3 text-center">
                                        {page.status === 'enabled' ? (
                                            <span className="inline-flex items-center gap-1.5 text-green-400 text-xs font-bold uppercase"><span className="h-2 w-2 rounded-full bg-green-500 animate-pulse"></span>Ativo</span>
                                        ) : (
                                            <span className="inline-flex items-center gap-1.5 text-red-400 text-xs font-bold uppercase"><span className="h-2 w-2 rounded-full bg-red-500"></span>Inativo</span>
                                        )}
                                    </td>
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        <button onClick={() => setSelectedPage(page)} className="p-2 hover:bg-gray-700 rounded-full transition-colors">
