                            ...
                            <tr key={page.id} className={`transition-colors ${selectedIds.has(page.id) ? 'bg-blue-900/20' : 'hover:bg-gray-900/50'}`}>
                                ...
                                <td className="px-4 py-3 text-right">
                                    <div className="flex items-center justify-end gap-2">
                                        <button onClick={() => handleStatusToggle(page)} className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 ${page.status === 'enabled' ? 'bg-blue-600' : 'bg-gray-600'}`}>
